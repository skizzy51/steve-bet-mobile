...
export default function OverallRacePage({ navigation, route }) {
    const window = useWindowDimensions();
    const dispatch = useDispatch()
    const { cash, serverError, token } = useSelector(state => state.user)
    const { totalEarnings, overallSelection, overallHorseList, stake } = route.params
    const [countdown, setCountdown] = useState(5)
    const [raceStart, setRaceStart] = useState(false)
    const [modalOpen, setModalOpen] = useState(false)

    const [racePlacement, setRacePlacement] = useState([...overallHorseList])
    ...
    function checkWinner () {
        if (
            racePlacement[0].name === overallSelection[0].name &&
            racePlacement[1].name === overallSelection[1].name &&
            racePlacement[2].name === overallSelection[2].name &&
            racePlacement[3].name === overallSelection[3].name
            ) {
                return true
        }else {
            return false
        ...
        <>
            ...
            <Modal animationType='slide' presentationStyle='pageSheet' visible={modalOpen}>
                <View style={{flex:1, paddingLeft:20, paddingRight:20}}>
                    ...
                    <View style={{flex:.35, justifyContent:'space-between', alignItems:'center', flexDirection:'row'}}>
                        <View>
                            <Image style={styles.winnerImage} source={firstImage}  />
                            <Text style={{fontSize:20, textAlign:'center', top:20, fontWeight:'600'}}>Winner - {racePlacement[0].name}</Text>
                        </View>
                        <View>
                            <Text style={styles.placementText}>2nd place - {racePlacement[1].name}</Text>
                            <Text style={styles.placementText}>3rd place - {racePlacement[2].name}</Text>
                            <Text style={styles.placementText}>4th place - {racePlacement[3].name}</Text>
                        </View>
                    </View>

                    <View style={{flex:.15, justifyContent:'space-between', borderTopColor:'grey', borderTopWidth:1}}>
                        <Text style={{textAlign:'center', fontSize:27, fontWeight:'500', paddingTop:15}}>Your selection</Text>

                        <View style={{flexDirection:'row', flexWrap:'wrap', justifyContent:'space-between'}}>
                            <Text style={{width:100, fontSize:17}}>1st - {overallSelection[0].name}</Text>
                            <Text style={{width:100, fontSize:17}}>2nd - {overallSelection[1].name}</Text>
                            <Text style={{width:100, fontSize:17}}>3rd - {overallSelection[2].name}</Text>
                            <Text style={{width:100, fontSize:17}}>4th - {overallSelection[3].name}</Text>
                        </View>
                    </View>
                    ...
                    <View style={{flex:.19, alignItems:'center', justifyContent:'center'}}>
                        {
                            checkWinner()
                            ? <Text numberOfLines={1} style={{fontSize:25, fontWeight:'600', color:'green'}}>You Win !!! ₦ {Platform.OS === 'android' ? totalEarnings.toFixed(2) : totalEarnings.toLocaleString("en-US", {minimumFractionDigits: 2, maximumFractionDigits : 2})}</Text>
                            : <Text style={{fontSize:40, fontWeight:'600', color:'crimson'}}>You Lose :(</Text>
                        }
                    </View>

                    <View style={{flex:.2}}>
                        <Pressable onPress={()=>navigation.replace('Overall-placement')} style={({pressed}) => pressed ? styles.modalBtnPressed : styles.modalBtn}>
